#!/bin/sh
# Check status of the background emb_tsne job started by nohup/emb_tsne/run.sh.
#
# Usage:
#   ./nohup/emb_tsne/check.sh
#   ./nohup/emb_tsne/check.sh -f    # follow latest log

ROOT="$(cd "$(dirname "$0")/../.." && pwd)"
PID_FILE="$ROOT/nohup/emb_tsne/job.pid"
META_FILE="$ROOT/nohup/emb_tsne/job.meta"
LOG_DIR="$ROOT/nohup/emb_tsne/logs"
...
echo "=== emb_tsne nohup job status ==="
